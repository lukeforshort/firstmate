...
run_session_start() {  # <home> <root> <path>
  ...
  # fm-harness.sh detects the primary harness from Layer-1 env markers
  # (CLAUDECODE/PI_CODING_AGENT/GROK_AGENT) before falling through to the ps
  # ancestry walk. These tests drive detection through the fake ps + FM_FAKE_HARNESS,
...
EOF
  ...
  # gh-axi is an npm-global firstmate tool that never lives in the standard
  # system dirs, so removing the fake stub genuinely makes it undetectable -
  # unlike node, which can be a system binary (/usr/bin/node) that survives the
  # fakebin removal and silently defeats the forced-missing intent.
  rm -f "$fakebin/gh-axi"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # Remove a firstmate npm-global tool (never a system binary) so the MISSING
  # line is forced deterministically; node can be a system /usr/bin binary that
  # would survive the fakebin removal.
  rm -f "$fakebin/gh-axi"
