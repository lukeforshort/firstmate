...
EOF
  ...
  # Drop a firstmate-only tool (never present on the ambient base PATH) rather
  # than node, which a packaged /usr/bin/node would otherwise still satisfy.
  rm -f "$fakebin/gh-axi"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # A firstmate-only tool is a stable forced-missing signal; the ambient base
  # PATH can still carry a packaged node, so removing node would not miss.
  rm -f "$fakebin/gh-axi"
