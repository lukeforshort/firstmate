...
state/               volatile runtime signals; gitignored
  ...
  .watch-arm.marker  live fm-watch-arm.sh in-progress signal (pid, fm-home, pid-identity); refreshed while arming, cleared on exit, read by the guards as fallback supervision-present evidence; never touch
  ...
  .last-daemon-beat  away-mode daemon liveness beacon, touched every cycle while the daemon runs; guard and session start alarm when .afk is present but it is stale or the daemon is gone
...
- `outcome: passed` or `checks-passed` - `checks-passed` means the PR is ready for review and the helper reports `done`; `passed` nominally means the PR merged or closed, but because no-mistakes has been seen reporting `passed` while the PR is still open, the helper confirms the terminal reading with a direct gh PR-state read: gh `merged`/`closed` reports `done: PR merged/closed`, gh `open` reports `working` (checks green, PR open awaiting merge - unlanded, so supervision continues), and an unresolvable PR reports `done` with a "PR merge state unverified" note.
