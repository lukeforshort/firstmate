...
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per bounded subprocess (no-mistakes query or gh PR-state read) inside fm-crew-state.sh
...
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, the primary turn-end guard, and the away-mode daemon-liveness check treat a watcher or daemon beacon as stale
...
FM_ARM_GRACE=30         # seconds the guard and turn-end guard accept a live fm-watch-arm marker as in-progress supervision before a stalled arm reads as not-in-progress
FM_PID_IDENTITY_RETRIES=3       # bounded retries of an empty/failed process-identity read in fm_watcher_lock_matches_pid, only while the pid stays alive; a genuine mismatch never retries
FM_PID_IDENTITY_RETRY_DELAY=0.05   # seconds between those identity-read retries
