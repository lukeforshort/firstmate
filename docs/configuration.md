...
FM_SYMPTOM_THRESHOLD=3   # occurrences a supervision-symptom class (state/.symptom-<class>.log) must reach before the guard/arm site prints its file-a-structural-fix recurrence advisory (bin/fm-symptom-lib.sh)
FM_SYMPTOM_DEBOUNCE_SECS=0   # default seconds within which a repeat symptom incident is collapsed into the same episode and not counted; a per-call debounce wins over this
FM_SYMPTOM_GUARD_DEBOUNCE=120   # seconds fm-guard.sh debounces its pull-based worktree-tangle symptom so one tangle episode counts once, not once per fleet action
FM_SYMPTOM_NOW=          # epoch override for deterministic symptom-tracker tests
